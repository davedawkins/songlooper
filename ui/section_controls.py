...
class SectionControlPanel(ttk.LabelFrame):
    ...
    def setup_ui(self):
        ...
        ttk.Label(self, text="Start (s):").grid(row=1, column=0, sticky=tk.W, padx=(0, 5), pady=5)
        self.ste = ttk.Entry(self, textvariable=self.app.stt, width=8)
        ...
        ttk.Label(self, text="End (s):").grid(row=1, column=2, sticky=tk.W, padx=(5, 5), pady=5)
        self.ene = ttk.Entry(self, textvariable=self.app.ent, width=8)
    ...
    def new_section(self):
        ...
        # Create new section with current time field values
        new_section = Section(
            name=new_name,
            start_time=self.app.stt.get(),
            end_time=self.app.ent.get()
        )
    ...
    def save_section(self):
        ...
        if section_name == "Full Song":
            ...
            for s in self.app.eng.current_song.sections:
                if s.name == section_name:
                    s.name = new_name
                    s.start_time = self.app.stt.get()
                    s.end_time = self.app.ent.get()
                    break
    ...
    def on_time_field_change(self, event):
        ...
        try:
            start_time = float(self.app.stt.get())
            end_time = float(self.app.ent.get())
            ...
            self.app.stt.set(round(self.app.stt.get(), 2))
            self.app.ent.set(round(self.app.ent.get(), 2))
